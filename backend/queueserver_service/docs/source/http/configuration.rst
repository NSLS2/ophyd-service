...
.. currentmodule:: queueserver_service.http
...
respectively. The configuration does not enable public access. ::
    ...
    authentication:
      providers:
        - provider: ldap
          authenticator: queueserver_service.http.authenticators:LDAPAuthenticator
          args:
...
scopes from ``unauthenticated_single_user`` role::
    ...
    api_access:
      policy: queueserver_service.http.authorization:BasicAPIAccessControl
      args:
        roles:
...
users lose access to the Queue Server. ::
    ...
    api_access:
      policy: queueserver_service.http.authorization:ServerBasedAPIAccessControl
      args:
        instrument: srx
...
the following policy configuration sets the returned group name to ``test_user``::
    ...
    resource_access:
      policy: queueserver_service.http.authorization:DefaultResourceAccessControl
      args:
