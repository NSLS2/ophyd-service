...
Substitute `<generated-api-key>` for the generated API key::
  ...
  QSERVER_HTTP_SERVER_SINGLE_USER_API_KEY=<generated-api-key> uvicorn --host localhost --port 60610 queueserver_service.http.server:app
...
and the path to the config file passed to the server::
    ...
    QSERVER_HTTP_SERVER_CONFIG=config.yml uvicorn --host localhost --port 60610 queueserver_service.http.server:app
...
override settings passed as environment variables. ::
    ...
    QSERVER_HTTP_SERVER_CONFIG=~/.qserver/http/config.yml uvicorn --host localhost --port 60610 queueserver_service.http.server:app

    # Multiple config files in the directory '~/.config/qserver/http'
    QSERVER_HTTP_SERVER_CONFIG=~/.config/qserver/http uvicorn --host localhost --port 60610 queueserver_service.http.server:app
