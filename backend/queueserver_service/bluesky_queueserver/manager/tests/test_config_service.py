...
# The fix keeps the list AND sets _config_service_unlock_failed; the next
# env-open sync releases the stale set first, then locks for the new env.
...
def _manager_lock_stub(client, *, devices=("m1", "det1")):
    ...
    manager._config_service_settings = SimpleNamespace(enabled=True)
    manager._config_service_lock_item_id = "env:test-uid"
    manager._config_service_locked_devices = []
    manager._config_service_unlock_failed = False
...
@pytest.mark.asyncio
async def test_unlock_failure_keeps_list_and_sets_flag():
    """Env-close path: the unlock raises, but the locked list must survive so
    the next env-open can reconcile (clearing it would orphan server locks)."""
    ...
    assert manager._config_service_unlock_failed is True


@pytest.mark.asyncio
async def test_unlock_failure_suppressed_keeps_list_and_sets_flag():
    """Env-destroy path (suppress_errors=True): no raise, but the stale list
...
@pytest.mark.asyncio
async def test_unlock_success_clears_list_and_flag():
    client = _LockClientStub()
    manager = _manager_lock_stub(client)
    manager._config_service_locked_devices = ["m1"]
    manager._config_service_unlock_failed = True

    await manager._unlock_config_service_devices()

    assert manager._config_service_locked_devices == []
    assert manager._config_service_unlock_failed is False
    assert client.unlock_calls == [(["m1"], "env:test-uid")]


@pytest.mark.asyncio
async def test_env_open_reconciles_stale_locks(_stub_env_open_sync):
    """THE P0.1 regression: after a failed unlock, the next env-open must
    release the stale set and then LOCK the new environment's devices —
    pre-fix it silently skipped locking forever."""
    client = _LockClientStub()
    ...
    assert client.unlock_calls == [(["old1", "old2"], "env:test-uid")]
...
async def test_env_open_skips_relock_within_same_env(_stub_env_open_sync):
    """Locks are acquired once per env: a re-sync with locks already held
    (and no unlock failure) must NOT lock again."""
    client = _LockClientStub()
    manager = _manager_lock_stub(client)
    manager._config_service_locked_devices = ["m1", "det1"]
    manager._config_service_unlock_failed = False
