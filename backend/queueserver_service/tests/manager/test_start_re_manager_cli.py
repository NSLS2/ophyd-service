...
from queueserver_service import gen_list_of_plans_and_devices
from queueserver_service.manager.config import default_existing_pd_fln, default_user_group_pd_fln
...
def test_start_re_manager_console_output_1(re_manager_cmd, console_print, console_zmq):  # noqa: F811
    ...
    def check_output_contents(collected_stdout):
        ...
        assert "queueserver_service.manager.manager manager:" in collected_stdout
        assert "queueserver_service.manager.profile_ops profile_ops:" in collected_stdout
        # Logging from Worker
        assert "queueserver_service.manager.worker worker:" in collected_stdout
    ...
    assert "queueserver_service.manager.output_streaming" in streamed_stderr
...
def _get_expected_settings_default_1(_1, _2):
    ...
    if use_ip_kernel:
        ...
        startup_dir = "/queueserver_service/profile_collection_sim/"
        ipython_dir = None
        startup_profile = None
        user_group_permissions_path = "/queueserver_service/profile_collection_sim"
        existing_plans_and_devices_path = "/queueserver_service/profile_collection_sim"
...
def func_for_test_check_env():
    from queueserver_service import is_re_worker_active, is_ipython_mode
