...
# (upstream module: bluesky_queueserver_api/zmq/aio.py).
...
class REManagerAPI(ReManagerComm_ZMQ_Async, API_Async_Mixin):
    """
    Asynchronous 0MQ API client for RE Manager. In-tree port of
    ``bluesky_queueserver_api.zmq.aio.REManagerAPI`` communicating over the
    service's own 0MQ comms.
    """
